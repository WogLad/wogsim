...
var MAX_ENTITIES_LIMIT: number = 500000; // Maximum number of concurrent entities in the world
...
function incrementEntityCount(e: Entity): void {
    entityCounts.total++;
    switch (e.entityType) {
        case ENTITY_TYPE_WOODCUTTER: entityCounts.woodcutter++; break;
        case ENTITY_TYPE_FISHERMAN: entityCounts.fisherman++; break;
        case ENTITY_TYPE_MINER: entityCounts.miner++; break;
        case ENTITY_TYPE_FARMER: entityCounts.farmer++; break;
        case ENTITY_TYPE_SHEEP: entityCounts.sheep++; break;
        case ENTITY_TYPE_COW: entityCounts.cow++; break;
        case ENTITY_TYPE_WOLF: entityCounts.wolf++; break;
    }
}

function decrementEntityCount(e: Entity): void {
    entityCounts.total--;
    switch (e.entityType) {
        case ENTITY_TYPE_WOODCUTTER: entityCounts.woodcutter--; break;
        case ENTITY_TYPE_FISHERMAN: entityCounts.fisherman--; break;
...
function findWasmPath(startX: number, startY: number, endX: number, endY: number, outBuffer: Int32Array): number {
...
function init(): void {
    ...
    for (var x = 0; x < X_TILES; x++) {
        ...
        for (var y = 0; y < Y_TILES; y++) {
            ...
            if (usePreprocessed) {
                ...
                tileTypeArray[idx] = TILE_TYPES.indexOf(typeStr as TileType);
            } else {
    ...
    for (var x = 0; x < X_TILES; x++) {
        for (var y = 0; y < Y_TILES; y++) {
            ...
            if (objId > 0) tile.worldObjects.push(new WorldObject(OBJ_NAMES[objId]));
            let itemId = itemArr[idx];
            if (itemId > 0) tile.items.push(new Item(ITEM_NAMES[itemId]));
        ...
        for (let speedStep = 0; speedStep < SIMULATION_SPEED; speedStep++) {
            ...
            for (var i = 0; i < entities.length; i++) {
                ...
                if (e.moveDelay === undefined) {
                    var baseDelay = 15;
                    if (entities.length > 30000) baseDelay = 60;
                    else if (entities.length > 20000) baseDelay = 30;
                    else if (entities.length > 10000) baseDelay = 20;
