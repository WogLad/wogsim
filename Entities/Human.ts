class Human extends Entity {
    ...
    constructor(professionLetter: string, customGenome?: Genome) {
    ...
    move: ((currentX: number, currentY: number) => Vector2) | null = (currentX, currentY) => {
        ...
        if (this.moveQueue.length > 0) {
            ...
            if (receptive && !isBusy && entities.length < currentLimit) {
                ...
                if (partnerPos) {
                    ...
                    if (Math.abs(currentX - partnerPos.x) <= 1 && Math.abs(currentY - partnerPos.y) <= 1) {
                        ...
                        let partner = partnerTile.entities.find(e => e !== this && e instanceof Human && e.isLiving) as Human;
                                ...
                                for (let dy = -1; dy <= 1 && !spawned; dy++) {
                                    ...
                                    if (world[bx] && world[bx][by]) {
                                        ...
                                        if (bTile.canBeTraversed() && bTile.entities.length < entLimit && bTile.worldObjects.length === 0) {
                                            ...
                                            if (Math.random() < 0.20) {
                                                ...
                                                for (let ent of entities) {
                                                    if (ent.entity instanceof Human) {
                                                        let letter = (ent.entity as Human).professionLetter;
                                                        if (letter === "W") counts.W++;
                                                        else if (letter === "F") counts.F++;
                                                        else if (letter === "M") counts.M++;
                                                        else if (letter === "P") counts.P++;
                                                    }
                                                }
                                                let minProf = "W";
                                                let minVal = Infinity;
                                                for (let prof of ["W", "F", "M", "P"] as const) {
                                            ...
                                            let baby: Human;
                                            if (babyLetter === "W") baby = new Woodcutter(babyGenome);
                                            else if (babyLetter === "F") baby = new Fisherman(babyGenome);
                                            else if (babyLetter === "M") baby = new Miner(babyGenome);
                                            else baby = new Farmer(babyGenome);
            ...
            if (this.hunger > 80 || (hasResources && (isFull || this.stateText === "Returning to Storage" || this.stateText === "Finding Food"))) {
                if (this.hunger > 80) this.stateText = "Finding Food";
                else this.stateText = "Returning to Storage";
                ...
                if (Math.abs(currentX - targetPos.x) <= 1 && Math.abs(currentY - targetPos.y) <= 1) {
                    if (this.stateText === "Finding Food") this.stateText = "Eating";
                    else this.stateText = "Depositing Resources";
                                ...
                                for (var hy = targetPos.y - d; hy <= targetPos.y + d && !housePlaced; hy++) {
                                    if (world[hx] && world[hx][hy]) {
                                        ...
                                        if (hTile.canBeTraversed() && hTile.worldObjects.length === 0 && hTile.type !== TileType.SAND && hTile.type !== TileType.SNOW) {
                                            ...
                                            //@ts-ignore
                                            if (typeof drawTileToOffscreen === "function") drawTileToOffscreen(hx, hy);
                                        }
                                    }
                                }
                            }
