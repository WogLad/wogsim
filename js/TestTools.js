...
class TestTools {
    ...
    static updateInspectorLive() {
        ...
        if (ticksEl)
            ticksEl.innerText = `${ent.ticksAlive} / ${Math.round(ent.maxAge)}`;
        // Genetics live update
        const geneLifespanEl = document.getElementById("liveInspectorGeneLifespan");
        if (geneLifespanEl)
            geneLifespanEl.innerText = ent.genome ? `${ent.genome.lifespanGene.toFixed(2)}x` : "1.00x";
        const geneHungerEl = document.getElementById("liveInspectorGeneHunger");
        if (geneHungerEl)
            geneHungerEl.innerText = ent.genome ? `${ent.genome.hungerRateGene.toFixed(2)}x` : "1.00x";
        const geneSpeedEl = document.getElementById("liveInspectorGeneSpeed");
        if (geneSpeedEl)
            geneSpeedEl.innerText = ent.genome ? `${ent.genome.speedGene.toFixed(2)}x` : "1.00x";
        ...
        for (let i = 0; i < totalEntities; i++) {
            ...
            if (ent.inventory) {
                for (let inv of ent.inventory) {
                    let itemName = inv.item ? inv.item.name : (inv.name || "");
                    if (["raw_meat", "cooked_meat", "fish", "bread", "wheat", "apple", "berry"].includes(itemName.toLowerCase()))
                        totalFood += inv.count;
                    else if (["log", "stone", "iron_ore", "wood"].includes(itemName.toLowerCase()))
                        totalMaterials += inv.count;
                }
            }
            // Activity
            let state = ent.stateText || "Unknown";
            if (state.includes("Working") || state.includes("Gather") || state.includes("Chop") || state.includes("Min") || state.includes("Farm") || state.includes("Fish"))
                working++;
            else if (state.includes("Sleep"))
                sleeping++;
            else if (state.includes("Idle") || state.includes("Wander"))
                idle++;
            else if (state.includes("Seeking Mate"))
                seekingMate++;
        }
