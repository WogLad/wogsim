    ...
    static saveGame() {
        ...
        for (let x = 0; x < X_TILES; x++) {
            ...
            for (let y = 0; y < Y_TILES; y++) {
                ...
                col.push({
                    type: tile.type,
                    worldObjects: tile.worldObjects,
        ...
        try {
            ...
            for (let x = 0; x < X_TILES; x++) {
                ...
                for (let y = 0; y < Y_TILES; y++) {
                    ...
                    // Restore items
                    tile.items = savedTile.items.map((itData) => {
                        //@ts-ignore
                    ...
                    tile.worldObjects = savedTile.worldObjects.map((woData) => {
                        //@ts-ignore
            ...
            for (let savedEnt of data.entities) {
                ...
                if ('lastFeedTime' in ent) {
                    ent.lastFeedTime = performance.now() - Math.random() * 20000;
                }
                // Fix inventory to ensure it's Item objects
                if (ent.inventory) {
                    ent.inventory = ent.inventory.map((inv) => {
                        let count = inv.count;
                        //@ts-ignore
