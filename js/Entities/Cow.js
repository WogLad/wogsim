...
class Cow extends Entity {
    constructor(customGenome) {
        ...
        this.move = (currentX, currentY) => {
            ...
            // 1. Run away from Wolf (within 5 tiles) using findNearest for efficiency
            var nearestWolf = this.findNearest(currentX, currentY, 5, (tile) => {
            ...
            if (receptive && entities.length < currentLimit) {
                let partnerPos = this.findNearest(currentX, currentY, 8, (tile) => {
                    for (var ei = 0; ei < tile.entities.length; ei++) {
                        ...
                        if (e !== this && e.entityType === ENTITY_TYPE_COW && e.isLiving) {
                ...
                if (partnerPos) {
                    ...
                    if (Math.abs(currentX - partnerPos.x) <= 1 && Math.abs(currentY - partnerPos.y) <= 1) {
                        ...
                        let partner = partnerTile.entities.find(e => e !== this && e.entityType === ENTITY_TYPE_COW && e.isLiving);
                        if (partner) {
                            let spawned = false;
        ...
        this.entityType = ENTITY_TYPE_COW;
        this.stateText = "Grazing";
